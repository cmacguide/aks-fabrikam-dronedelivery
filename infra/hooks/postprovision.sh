...
# Generated by Copilot - Enable debug mode if requested
if [[ "${1}" == "--debug" ]] || [[ "${AZD_DEBUG}" == "true" ]]; then
  set -x
...
# Generated by Copilot - Debug function to check function availability
debug_functions() {
  log_info "Checking function availability..."

  local functions=("get_deployment_outputs" "configure_kubectl" "configure_current_user_keyvault_permissions"
    "generate_certificates" "setup_kubernetes_rbac" "install_agic"
    "configure_gitops" "setup_monitoring" "configure_aks_keyvault_permissions"
    "validate_deployment" "show_next_steps")

  for func in "${functions[@]}"; do
    if declare -f "$func" >/dev/null; then
      log_success "Function $func is defined"
    else
      log_error "Function $func is NOT defined"
    fi
  done
}

# Generated by Copilot - Simplified test function
test_basic_connectivity() {
  log_info "Testing basic connectivity..."

  # Test Azure CLI
  if az account show --output none 2>/dev/null; then
    log_success "Azure CLI is working"
  else
    log_error "Azure CLI is not working"
    return 1
  fi

  # Test kubectl if available
  if command -v kubectl >/dev/null 2>&1; then
    if kubectl get namespaces --output none 2>/dev/null; then
      log_success "kubectl is working"
    else
      log_warning "kubectl is not configured or cluster not accessible"
    fi
  else
  ...
  if [ -n "$KEY_VAULT_NAME" ]; then
    if az keyvault list --resource-group "$SECURITY_RG" --output none 2>/dev/null; then
      log_success "Key Vault access is working"
    else
      log_warning "Key Vault access may have issues"
    fi
  fi
}
...
# Generated by Copilot - Function to generate K8s manifests with environment variables
generate_k8s_manifests() {
  log_info "Generating Kubernetes manifests with environment variables..."

  # Diretório dos templates e saída
  TEMPLATE_DIR="./infra/cluster-manifests"
  OUTPUT_DIR="./infra/cluster-manifests/generated"

  # Criar diretório de saída se não existir
  mkdir -p "$OUTPUT_DIR"

  # Carregar variáveis do azd env
  source <(azd env get-values)

  # Exportar variáveis necessárias para envsubst
  export APP_NAME="${appName:-fabrikam-dronedelivery}"
  export ENVIRONMENT_NAME="${environmentName:-dev}"
  export K8S_RBAC_ENTRA_ADMIN_GROUP_OBJECT_ID="${k8sRbacEntraAdminGroupObjectID}"
  ...
  log_info "Processing templates with variables:"
  log_info "  - APP_NAME: $APP_NAME"
  log_info "  - ENVIRONMENT_NAME: $ENVIRONMENT_NAME"
  log_info "  - K8S_RBAC_ENTRA_ADMIN_GROUP_OBJECT_ID: $K8S_RBAC_ENTRA_ADMIN_GROUP_OBJECT_ID"

  # Processar todos os templates
  for template_file in "$TEMPLATE_DIR"/*.template.yaml; do
    if [ -f "$template_file" ]; then
      base_name=$(basename "$template_file" .template.yaml)
      output_file="$OUTPUT_DIR/${base_name}.yaml"

      log_info "Processing $(basename "$template_file")..."
